...
public class Scope64 {
    ...
    public static double[] sRGB_2_XYZ(int r, int g, int b) {
        double R = r / 255.0;
        double G = g / 255.0;
        double B = b / 255.0;
    ...
    public static double[] sRGB_2_LAB(int r, int g, int b) {
        double[] xyz = sRGB_2_XYZ(r, g, b);
        double X = xyz[0] / 0.964212;
        double Y = xyz[1] / 1.000000;
        double Z = xyz[2] / 0.825188;
    ...
    static class SelectionFrame extends JFrame {
        ...
        private static void startRealTimeUpdate(Rectangle selection) {
            ...
            if (sharedRobot == null) {
                // System.err.println("Robot non disponible, impossible de capturer l'ecran.");
                return;
            }
            
            refreshTimer = new Timer(100, e -> {
                lastCapture = sharedRobot.createScreenCapture(lastSelection);
                BufferedImage frame = showZoneSystem
                    ? createZoneSystem(500)
        ...
        private static void computeLabStats(BufferedImage image) {
            ...
            for (int y = 0; y < image.getHeight(); y++) {
                for (int x = 0; x < image.getWidth(); x++) {
                    ...
                    if (L < minLstar) minLstar = L;
                    if (L > maxLstar) maxLstar = L;
                    if (L < 3.0)  countLowL++;
                    if (L > 97.0) countHighL++;
                }
            }

            double avgRd = totalR / nbPixels;
            double avgGd = totalG / nbPixels;
            double avgBd = totalB / nbPixels;

            double[] avgLab = Scope64.sRGB_2_LAB(
                (int) Math.round(avgRd),
                (int) Math.round(avgGd),
                (int) Math.round(avgBd)
            );
            avgLstar = avgLab[0];
            avgAstar = avgLab[1];
            avgBstar = avgLab[2];
            ...
            double[] xy = Scope64.rgbToXy(
                (int) Math.round(avgRd),
                (int) Math.round(avgGd),
                (int) Math.round(avgBd)
            );
            colorTemp = Scope64.calculateColorTemperature(xy[0], xy[1]);
        }
        
        private static void drawStats(Graphics2D g2d, FontMetrics metrics, int size) {
            String lowLText = String.format("%.2f", percentLowL);
            g2d.setColor(percentLowL >= 1 ? Color.WHITE : new Color(128, 128, 128));
            drawText(g2d, metrics, lowLText, 10, 20);

            g2d.setColor(new Color(128, 128, 128));
            drawText(g2d, metrics, String.format("D  %.1f", dynamicRange), 10, 40);
            drawText(g2d, metrics, String.format("EV  %.1f", dynamicRangeEV), 10, 60);

            String colorTempText = (colorTemp < 0 || colorTemp > 10000) ? "K" : String.format("K  %.0f", colorTemp);
            drawText(g2d, metrics, colorTempText, 10, 80);

            String highLText = String.format("%.2f", percentHighL);
            g2d.setColor(percentHighL >= 1 ? Color.WHITE : new Color(128, 128, 128));
            drawText(g2d, metrics, highLText, size - metrics.stringWidth(highLText) - 10, 20);

            g2d.setColor(new Color(128, 128, 128));
            String[] colorTexts = getColorTexts();
            drawText(g2d, metrics, colorTexts[0], size - metrics.stringWidth(colorTexts[0]) - 10, 40);
            drawText(g2d, metrics, colorTexts[1], size - metrics.stringWidth(colorTexts[1]) - 10, 60);
        }
        ...
        private static BufferedImage createLabAxisImage(int size) {
            ...
            drawStats(g2d, metrics, size);
            
            g2d.dispose();
            return labImage;
        }

        private static BufferedImage createZoneSystem(int size) {
            ...
            // Stats communes — maintenant a jour grace a computeLabStats
            g2d.setFont(new Font("SansSerif", Font.PLAIN, 12));
            drawStats(g2d, g2d.getFontMetrics(), size);
        ...
        private static String[] getColorTexts() {
            if (lastCapture == null) return new String[] { "", "" };
            if (Math.abs(avgAstar) < 0.001 && Math.abs(avgBstar) < 0.001) return new String[] { "", "" };
        ...
        private static void drawLabAxis(Graphics2D g2d, int centerX, int centerY, int radius) {
            ...
            for (int y = centerY - radius; y <= centerY + radius; y++) {
                for (int x = centerX - thickness / 2; x <= centerX + thickness / 2; x++) {
                    ...
                    if (distance <= radius) {
                        ...
                        Color color = (y < centerY)
                            ? interpolateColor(centerColor, new Color(255, 0, 149), ratio)
                            : (y > centerY)
                                ? interpolateColor(centerColor, new Color(0, 155, 116), ratio)
                                : centerColor;
                        g2d.setColor(color);
            ...
            for (int x = centerX - radius; x <= centerX + radius; x++) {
                for (int y = centerY - thickness / 2; y <= centerY + thickness / 2; y++) {
                    ...
                    if (distance <= radius) {
                        float ratio = (float)(distance / radius);
                        Color color = (x > centerX)
                            ? interpolateColor(centerColor, new Color(255, 238, 0), ratio)
                            : (x < centerX)
                                ? interpolateColor(centerColor, new Color(0, 96, 255), ratio)
                                : centerColor;
                        g2d.setColor(color);
                        g2d.fillRect(x, y, 2, 2);
                    }
        ...
        private static void drawText(Graphics2D g2d, FontMetrics metrics, String text, int x, int y) {
        ...
        private static void plotLabPoints(Graphics2D g2d, BufferedImage image, int centerX, int centerY, int maxRadius) {
            ...
            // Calcul des stats via la methode commune
            computeLabStats(image);
        ...
        public static void launchSelection() {
            ...
            if (refreshTimer != null) refreshTimer.stop();
            if (resultFrame != null)  resultFrame.setVisible(false);
            
            if (sharedRobot == null) {
                // System.err.println("Robot non disponible.");
                return;
            }
            ...
            for (GraphicsDevice screen : screens) {
                ...
                BufferedImage screenImage = sharedRobot.createScreenCapture(screenBounds);
